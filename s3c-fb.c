...
static const struct of_device_id s3c_fb_driver_matchs[];
...
static int s3c_fb_probe(struct platform_device *pdev)
{
    ...
    if(!display_control)    {    
        printk(KERN_ERR" s3c-fb ioremap fail\n");
        return -ENOMEM;
    }    
    writel(0x2 , display_control);

#else
	/* tradition form platform_device get info */
	...
	/* get the soc dispaly controller information */
    match = of_match_device(s3c_fb_driver_matchs, &pdev->dev);
    if(!match){
        printk(KERN_ERR" s3c-fb of_match_device fail\n");
        return -ENODEV;
    }
	fbdrv = (struct s3c_fb_driverdata *)match->data;
	if (fbdrv->variant.nr_windows > S3C_FB_MAX_WIN) {
		dev_err(dev, "too many windows, cannot attach\n");
		return -EINVAL;
	}
	
	pd = devm_kzalloc(dev, sizeof(struct s3c_fb_platdata), GFP_KERNEL);
#else
    ...
    if(!pd->vtiming) {
        dev_err(dev, "no memory for fb_videomode\n");
        return -ENOMEM;
    }
    ...
    of_property_read_u32(np,"lower_margin" , &pd->vtiming->lower_margin);
    of_property_read_u32(np,"vsync-len" , &pd->vtiming->vsync_len);
    of_property_read_u32(np,"hsync-len" , &pd->vtiming->hsync_len);
    of_property_read_u32(np,"pixclock" , &pd->vtiming->pixclock);


    printk(KERN_INFO"right_margin   :%d\n", pd->vtiming->right_margin);
    printk(KERN_INFO"left_margin    :%d\n", pd->vtiming->left_margin);
    printk(KERN_INFO"upper_margin   :%d\n", pd->vtiming->upper_margin);
    printk(KERN_INFO"lower_margin   :%d\n", pd->vtiming->lower_margin);
    printk(KERN_INFO"vsync-vsync    :%d\n", pd->vtiming->vsync_len);
    printk(KERN_INFO"hsync-vsync    :%d\n", pd->vtiming->hsync_len);

    pd->win[0] = devm_kzalloc(dev, sizeof(struct s3c_fb_pd_win), GFP_KERNEL);
    if(!pd->win[0]) {
        dev_err(dev, "no memory for s3c_fb_pd_win\n");
        return -ENOMEM;
    }

    of_property_read_u16(np,"xres" , &pd->win[0]->xres);
    of_property_read_u16(np,"yres" , &pd->win[0]->yres);
    of_property_read_u16(np,"max_bpp" , &pd->win[0]->max_bpp);
    of_property_read_u16(np,"default_bpp" , &pd->win[0]->default_bpp);
	
	pd->vtiming->xres = pd->win[0]->xres;
	pd->vtiming->yres = pd->win[0]->yres;

    printk(KERN_INFO"xres       :%d\n", pd->win[0]->xres);
    printk(KERN_INFO"yres       :%d\n", pd->win[0]->yres);
    printk(KERN_INFO"max_bpp        :%d\n", pd->win[0]->max_bpp);
    printk(KERN_INFO"default_bpp    :%d\n", pd->win[0]->default_bpp);

    of_property_read_u32(np, "hsync-active", &tmp);
    pd->vidcon1 |= ((tmp&0x1) ? VIDCON1_INV_HSYNC : 0);

    of_property_read_u32(np, "vsync-active", &tmp);
    pd->vidcon1 |= ((tmp&0x1) ? VIDCON1_INV_VSYNC : 0);

    of_property_read_u32(np, "vidout", &tmp);
    pd->vidcon0 |= ((tmp&0x7) << VIDCON0_VIDOUT_SHIFT);

    of_property_read_u32(np, "pnrmode", &tmp);
    pd->vidcon0 |= ((tmp&0x3) << VIDCON0_PNRMODE_SHIFT);

    printk(KERN_INFO"vidcon0            :%x\n", pd->vidcon0);
    printk(KERN_INFO"vidcon1            :%x\n", pd->vidcon1);
#endif
...
static const struct of_device_id s3c_fb_driver_matchs[] = {
    { .compatible = "samsung,64xx-fb",    .data = &s3c_fb_data_64xx    },   
    { .compatible = "samsung,s5pv210-fb", .data = &s3c_fb_data_s5pv210 },
    { .compatible = "samsung,exynos4-fb", .data = &s3c_fb_data_exynos4 },
    { .compatible = "samsung,exynos5-fb", .data = &s3c_fb_data_exynos5 },
    { .compatible = "samsung,s3c2443-fb", .data = &s3c_fb_data_s3c2443 },
    { /* sentinel */ }
};
...
static struct platform_driver s3c_fb_driver = {
	...
	.driver		= {
		...
		.pm	= &s3cfb_pm_ops,
#endif
		.of_match_table = of_match_ptr(s3c_fb_driver_matchs),
	},
};
